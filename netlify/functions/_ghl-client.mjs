 ...
 * This bypasses RLS — only use server-side in Netlify functions.
 */
...
export async function getGHLToken(locationId) {
  ...
  if (expiresAt < fiveMinFromNow) {
    const refreshed = await refreshGHLToken(locationId);
 ...
 * Handles token retrieval and sets required headers.
 */
export async function makeGHLRequest(endpoint, method, body, locationId) {
  ...
  // Log rate limit headers as warnings
  ...
  if (remaining !== null) {
    console.log(`[GHL Rate Limit] Remaining/sec: ${remaining}, Remaining/day: ${remainingDay}`);
  }
