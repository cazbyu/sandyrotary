...
export function Bulletins() {
  ...
  const handleShare = async (bulletin: Bulletin, e: React.MouseEvent) => {
    e.stopPropagation();
      ...
      <Layout showHeader={false}>
        <div className="min-h-screen bg-[#F5F7FA]">
          <div className="bg-[#1B2A4A] px-4 py-4 flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-white flex-1">Announcements</h1>
          </div>
        ...
        <div className="p-4 space-y-4">
          {bulletins.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-600">No bulletins yet</p>
            </div>
          ) : (
            bulletins.map((bulletin) => (
              ...
              >
                ...
                <div className="p-4">
                  <h3 className="font-bold text-gray-800 text-lg mb-2">{bulletin.title}</h3>
                  ...
                  <p className="text-gray-700 mb-4">{truncateText(bulletin.body, 200)}</p>

                  <button
                    onClick={(e) => handleShare(bulletin, e)}
