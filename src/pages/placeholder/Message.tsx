...
export function Message() {
  ...
  const loadMembers = async () => {
    try {
      ...
      const membersWithPhone = (data || []).filter(
        (member) => member.preferred_phone || member.mobile_phone
      );

      setMembers(membersWithPhone);
      setFilteredMembers(membersWithPhone);
    } catch (error) {
    ...
    <Layout showHeader={false}>
      <div className="min-h-screen bg-[#F5F7FA] pb-20">
        <div className="bg-[#1B2A4A] px-4 py-4 flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-white flex-1">Message a Member</h1>
        </div>
        ...
        <div className="p-4">
          ...
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-gray-600">No members available to message</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <div className="text-sm text-gray-600">
                      {member.preferred_phone || member.mobile_phone}
                    </div>
                  </div>
                </button>
              ))}
