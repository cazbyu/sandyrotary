...
export function BulletinForm() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from('0012-sr-bulletin-images').getPublicUrl(filePath);
      ...
      <div className="min-h-screen bg-[#F5F7FA] pb-20">
        <div className="bg-[#1B2A4A] px-4 py-4 flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold text-white flex-1">
            {existingId ? 'Edit Bulletin' : 'New Bulletin'}
          </h1>
        </div>
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-md p-4">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <span className="text-sm font-semibold text-gray-800">Publish bulletin</span>
            </label>
            <p className="text-xs text-gray-600 mt-1 ml-8">
              Published bulletins are visible to all members and can be shared publicly
            </p>
          </div>
